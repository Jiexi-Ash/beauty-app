...
export const createService = mutation({
  ...
  handler: async (
    ctx,
    { categoryName, description, duration, name, price, primaryImageStorageId },
  ) => {
...
export const updateService = mutation({
  ...
  handler: async (
    ...
    if (
      primaryImageStorageId &&
      service.primaryImageStorageId !== primaryImageStorageId
    ) {
    ...
    if (galleryImageIds) {
      ...
      const existingIds = new Set(
        existingGallery.map((img) => img.imageStorageId),
      );
      const incomingIds = new Set(galleryImageIds);

      const toDelete = existingGallery.filter(
        (img) => !incomingIds.has(img.imageStorageId),
      );
...
export const deleteService = mutation({
  ...
  handler: async (ctx, { serviceId }) => {
    ...
    if (!service)
      throw new ConvexError("Service not found with the provided ID.");
...
export const toggleVisibility = mutation({
  ...
  handler: async (ctx, { serviceId, visibility }) => {
    ...
    if (!business)
      throw new ConvexError(
        "Business not found. Please create a business before deleting services.",
      );
...
export const getServiceById = query({
  ...
  handler: async (ctx, { id }) => {
    ...
    const serviceWithPrimaryImage = await ctx.storage.getUrl(
      service.primaryImageStorageId,
    );
    const serviceImages = await ctx.db
      .query("serviceImages")
